  ...
  register: (req, res) => {
    ...
    User.findOne({ name: username }, (err, user) => {
      if (err) {
        ...
        User.create(
          { name: username, password_hash: pw_hash(password) },
          (err, new_user) => {
            if (err) {
              send(res, 500, err);
            } else {
              console.log(
                `[db] Created new user(${new_user._id}) ${new_user.name}`
              );
              send(res, 200);
            }
          }
    ...
    Post.create(
      ...
      (err, new_post) => {
        if (err) {
          ...
          console.log(
            `[db] Created new post(${new_post._id}) ${new_post.title} in category ${new_post.category}`
          );
          send(res, 200);
        }
      }
            ...
            Post.updateOne(
              ...
              (err) => {
                if (err) {
                  ...
                  console.log(
                    `[db] User(${req.session.userid}) commented on post(${req.body.post_id})`
                  );
                  send(res, 200);
                }
              }
      ...
      if (err) {
        ...
        Comment.deleteOne({ _id: req.body.comment_id }, (err) => {
          if (err) {
            ...
            console.log(
              `[db] User(${req.session.userid}) deleted a comment(${req.body.comment_id}) on post(${post.id})`
            );
            send(res, 200);
          }
        });
    ...
    Event.create(
      ...
      (err, new_event) => {
        if (err) {
          ...
          console.log(
            `[db] Created new event(${new_event._id}) ${new_event.title}`
          );
          send(res, 200);
        }
      }
        ...
        Event.updateOne(
          ...
          (err) => {
            if (err) {
              ...
              console.log(
                `[db] User(${req.session.userid}) joined event(${req.body.event_id})`
              );
              send(res, 200);
            }
          }
        ...
        Event.updateOne(
          ...
          (err) => {
            if (err) {
              ...
              console.log(
                `[db] User(${req.session.userid}) left event(${req.body.event_id})`
              );
              send(res, 200);
            }
          }
    ...
    Poll.create(
      ...
      (err, new_poll) => {
        if (err) {
          ...
          console.log(
            `[db] Created new poll(${new_poll._id}) ${new_poll.prompt}`
          );
          send(res, 200);
        }
      }
        ...
        Poll.updateOne(
          ...
          (err) => {
            if (err) {
              ...
              console.log(
                `[db] User(${req.session.userid}) voted on poll(${req.body.poll_id})`
              );
              send(res, 200);
            }
          }
        ...
        Poll.updateOne(
          ...
          (err) => {
            if (err) {
              ...
              console.log(
                `[db] User(${req.session.userid}) cleared vote on poll(${req.body.poll_id})`
              );
              send(res, 200);
            }
          }
