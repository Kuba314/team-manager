...
post_endpoints = [
    ...
    {endpoint: '/deleteUser',       auth: true , callback: handlers.deleteUser,     keys: []},                                                  // 404(user doesn't exist)
    ...
    {endpoint: '/comment',          auth: true , callback: handlers.comment,        keys: ['post_id', 'text']},                                 // 404(post not found)
    {endpoint: '/deleteComment',    auth: true , callback: handlers.deleteComment,  keys: ['comment_id']},                                      // 403(not logged in as author), 404(comment not found)

    {endpoint: '/addEvent',         auth: true , callback: handlers.addEvent,       keys: ['title', 'body', 'time', 'location']},               //
    {endpoint: '/deleteEvent',      auth: true , callback: handlers.deleteEvent,    keys: ['event_id']},                                        // 403(not logged in as author), 404(event doesn't exist)
    {endpoint: '/editEvent',        auth: true , callback: handlers.editEvent,      keys: ['event_id', 'title', 'body', 'time', 'location']},   // 403(not logged in as author), 404(event doesn't exist)
    {endpoint: '/joinEvent',        auth: true , callback: handlers.joinEvent,      keys: ['event_id']},                                        // 404(event doesn't exist), 409(already joined)
    {endpoint: '/leaveEvent',       auth: true , callback: handlers.leaveEvent,     keys: ['event_id']},                                        // 404(event doesn't exist), 409(not attending this event)

    {endpoint: '/addPoll',          auth: true , callback: handlers.addPoll,        keys: ['prompt', 'options']},                               //
    {endpoint: '/deletePoll',       auth: true , callback: handlers.deletePoll,     keys: ['poll_id']},                                         // 403(not logged in as author), 404(poll not found)
    {endpoint: '/vote',             auth: true , callback: handlers.vote,           keys: ['poll_id', 'option']},                               // 403(not logged in as author), 404(poll not found)
    {endpoint: '/clearVote',        auth: true , callback: handlers.clearVote,      keys: ['poll_id']},                                         // 403(not logged in as author), 404(poll not found)
]
